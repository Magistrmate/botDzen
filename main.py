        ...
        async for member in app.get_chat_members(config.chat_dzen):
            if member.user.is_self or member.user.is_bot is False:
                ...
                cursor.execute('INSERT INTO users (user_id, is_contact, is_mutual_contact, is_deleted, is_verified, '
                               ...
                               'last_online_date, next_offline_date, username, language_code, emoji_status, dc_id, '
                               'phone_number, photo, restriction, mention, join_date) VALUES (?,?,?)',
                               (user_id, is_contact, is_mutual_contact, is_deleted, is_verified, is_restricted, is_scam,
                                is_fake, is_premium, first_name, last_name, status, last_online_date, next_offline_date,
                                username, language_code, emoji_status, dc_id, phone_number, photo, restriction, mention,
                                join_date))
        conn.commit()
